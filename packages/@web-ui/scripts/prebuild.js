 ...
 *
 * This script:
 * 1. Copies the entire lib folder from web-ui-dev to @web-ui; overwriting any existing lib folder.
...
async function main() {
  try {
    ...
    if (fs.existsSync(webUiLibDir)) {
      console.log('Removing existing lib directory...');
      fs.rmSync(webUiLibDir, { recursive: true, force: true });
    }

    // Copy the entire lib folder from web-ui-dev to @web-ui
    console.log('Copying lib folder from web-ui-dev to @web-ui...');
    copyDirectoryRecursive(webUiDevLibDir, webUiLibDir);

    // Rename index.ts/tsx to main.ts/tsx in the copied lib folder
    console.log('Renaming index.ts/tsx to main.ts/tsx...');
    const indexTsPath = path.join(webUiLibDir, 'index.ts');
    const indexTsxPath = path.join(webUiLibDir, 'index.tsx');
    const mainTsPath = path.join(webUiLibDir, 'main.ts');
    const mainTsxPath = path.join(webUiLibDir, 'main.tsx');
    ...
    const libFiles = fs.readdirSync(webUiLibDir);
